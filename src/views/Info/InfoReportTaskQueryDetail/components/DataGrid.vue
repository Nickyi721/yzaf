<template>
  <el-table show-summary :summary-method='getSummaries' sum-text='统计' :data="data" height="100%" class="table" @selection-change="_TableSelectionChange">
    ...
    <el-table-column v-for="(item,index) in headerData" :key="index" :prop="item.tnZdybbtid.toString()" align="center" min-width="150">
      ...
      <div v-show="!item.zdybbtiList.length" slot-scope="scope">
        <component :is='componentsList[item.tcLrfs]' class="input" v-model="scope.row[item.tnZdybbtid]" :disabled="!canEdit" size="mini" :type="[0,1].indexOf(item.tnSjlx)>-1?'number':'text'" :suffix-icon="item.tcLrfs==='1'?'el-icon-d-caret':''" :placeholder="item.tnSjlx | placeholder(item)" @input="computeValue(scope.row,item)" maxlength='item.tnSjlx | maxlength' />
      </div>
      <el-table-column v-for="(ceil,i) in item.zdybbtiList" :key="i" align="center" min-width="150" :prop="ceil.tnZdybbtid.toString()">
        ...
        <div slot-scope="scope">
          <component :is='componentsList[ceil.tcLrfs]' class="input" v-model="scope.row[ceil.tnZdybbtid]" :disabled="!canEdit" size="mini" :type="[0,1].indexOf(ceil.tnSjlx)>-1?'number':'text'" :suffix-icon="item.tcLrfs==='1'?'el-icon-d-caret':''" :placeholder="ceil.tnSjlx | placeholder(ceil)" :maxlength='ceil.tnSjlx | maxlength' @input="computeValue(scope.row,ceil)" />
        </div>
      </el-table-column>
    </el-table-column>
  </el-table>
    ...
    computeValue (row, item) {
      ...
      switch (item.tnSjlx) {
        case 0:
          ...
          break
        case 1:
          value = value.toString().substr(0, 13)
