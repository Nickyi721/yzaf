  ...
  methods: {
    setContent() {
      this.config.forEach(primary => {
        this.parseValue(primary)
        primary.children && primary.children.forEach((child, idx) => {
          this.parseValue(child)
          child.children && child.children.forEach((secondary, idx) => {
            this.parseValue(secondary)
          })
        })
      })
    },
    parseValue (item) {
      const value = this.formData[item.tnPgnrszbid]
      if (item.tnWtyylx === 1) {
        this.$set(this.formData, item.tnPgnrszbid, value ? value.split(',').map(v => parseInt(v)) : [])
      } else {
        this.$set(this.formData, item.tnPgnrszbid, value ? parseInt(value) : null)
      }
    }
  },
}
