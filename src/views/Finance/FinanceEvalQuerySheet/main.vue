      ...
      <div class="table_body">
        ...
        >
          ...
          <el-table-column label="扣分项目">
            <template slot-scope="scope">{{ _writeTableSecond(scope.row) }}</template>
          </el-table-column>
          <el-table-column label="存在问题" width="150px">
            <template slot-scope="scope">{{ _writeTableThird(scope.row) }}</template>
          </el-table-column>
          <el-table-column label="实际分值" width="80px" prop="tcZf">
            <template slot-scope="scope">{{ _writeTableForth(scope.row) }}</template>
          </el-table-column>
        </el-table>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    _ButtonExportClick() {
      let exportList = [];
      let widthList = document.getElementsByTagName("colgroup")[0].childNodes;
      for (let item of widthList) {
        this.tableConfig.colwidth.push({ wpx: item.width });
      }
      let fileName = `${JSON.parse(this.$route.query.curOrg).tcDwmc}安全计分表`;
      let thList = document.getElementsByTagName("thead")[0].childNodes[0]
        .childNodes; //表头
      let idx = []; //用于表头和表体一一对应
      thList.forEach((v, j) => {
      ...
      let tbodyList = Array.from(document.getElementsByTagName("tbody")).slice(
        0,
        2
      ); //合计为另外一个表
      tbodyList.forEach(e => {
        let trList = e.childNodes;
        let condition = trList.length > 1 ? trList.length - 1 : trList.length;
        trList.forEach((v, j) => {
          if (j < condition) {
            let temp = {};
            ...
            tdList.forEach((a, i) => {
              ...
              idx.forEach(arr => {
                if (i === arr) {
                  let key = thList[arr].textContent.trim();
                  temp[key] = text;
                }
              });
            });
    ...
    async getFinSecEvaScoreTable() {
      ...
      if (result.data && result.state === "1") {
        ...
        for (let j = 0; j < firstLevel.length; j++) {
          if (
            j < firstLevel.length - 1 &&
            firstLevel[j].tcPglx != firstLevel[j + 1].tcPglx
          ) {
            const obj = {
        ...
        if (thirdLevel.length > 0) {
          for (let m = 0; m < secondLevel.length; m++) {
            let arr = thirdLevel.filter(
              a =>
                a.tcPglx === secondLevel[m].tcPglx &&
                a.tcSjbm == secondLevel[m].tnNrbh
            );
          ...
          for (let m = 0; m < firstLevel.length; m++) {
            ...
            secondLevel.forEach(el => {
              if (el.length && el.length > 0) {
                if (
                  el[0].tcPglx === firstLevel[m].tcPglx &&
                  el[0].tcSjbm == firstLevel[m].tnNrbh
                ) {
                  arr = [...arr, ...el];
                }
              } else {
                if (
                  el.tcPglx === firstLevel[m].tcPglx &&
                  el.tcSjbm == firstLevel[m].tnNrbh
                ) {
                  arr = [...arr, el];
                }
              }
