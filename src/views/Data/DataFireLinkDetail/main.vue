  ...
  <CustomView>
    ...
    <div class="common-editable">
      <el-table stripe border height="100%" :data="list" size="mini" class="grid-table" header-cell-class-name="grid-table-hd">
        ...
        <el-table-column label="关联设备及故障信息项" min-width="200px">
          <select v-model="scope.row['tnMxid']" slot-scope="scope" >
            <option v-for="option in originList" :key="option.tnRcxjjbxxszid" :value="option.tnRcxjjbxxszid">{{option.tcXxxmc}}</option>
          </select>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    $init (options) {
      //console.log("adsda")
      this.formData.btxxxlx =this.$route.query.btxxxlx
      this.formData.mxxxxlx =this.$route.query.mxxxxlx
      //console.log(scope.row['tnXxxldid'],'scope')
      if (options) {
        if (options['1002']) {
          ...
          this.options.facilityType = [{ tcSjubm: '', tcSjnr: '请选择' }].concat(options['1002'])
        }
      }
    },
    // 事件 - 切换评估类型
    ...
    //     const result = await this.getFacilityList()
    //     const list =result.data.mxList
    ...
    async getEvalList () {
      const result = await Model.getEvalList({
        btxxxlx :this.formData.btxxxlx,
        mxxxxlx :this.formData.mxxxxlx,
      })
      if (result.data) {
        //console.log(result.data)
        this.list = result.data.btList

        this.originList=[{ tcSjubm: '', tcXxxmc: '请选择' }].concat(result.data.mxList)
        //this.getLinkList()
      }
    },
    async getFacilityList () {
      const result = await Model.getFacilityList({
        btxxxlx :this.formData.btxxxlx,
        mxxxxlx :this.formData.mxxxxlx,
      })
      return result.data || []
    ...
    async saveListData () {
      ...
      if (list.filter(row => !row.tnMxid).length) {
        this.$msgbox({
          message: '关联设备及故障信息项不能为空',
          type: 'warning'
        })
      } else {
        const data = list.map(row => ({
          tnSzlbid: row.tnRcxjjbxxszid,
          tnSzmxid: row.tnMxid,
        }))

        const result = await Model.saveListData({
           insertList:data,
           tcLbxxxlx:this.list['0'].tcXxxlx,
           tcMxxxxlx:this.originList['1'].tcXxxlx,
           tnLx:2
        })
        if (result.state === '1') {
          this.$message({
