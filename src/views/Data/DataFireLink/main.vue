<template>
  <CustomView>
    <CustomToolbar slot="top">
      ...
      <template>
      <el-dialog title="请关联表头和表信息类型" :visible.sync="dialogFormVisible"  :modal="false" style="width:50%;margin:0 auto;">
      <el-form :model="formData" >
      <el-form-item label="信息项类型" :label-width="formLabelWidth">
          <el-select v-model="formData['tcLbxxxlx']" placeholder="请选择表头类型">
             <el-option v-for="option in options['100206']" :key="option.tcSjubm" :label="option.tcSjnr" :value="option.tcSjubm"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="信息项类型名称" :label-width="formLabelWidth">
          <el-select v-model="formData['tcMxxxxlx']" placeholder="请选择表信息类型">
  ...
  methods: {
    ...
    $init (options) {
      if (options) {
        ...
        // console.log(this.evenNumbers,'daa')
    ...
    _ButtonDeleteClick() {
      ...
      if (this.selection.length > 0) {
        this.$confirm('是否删除当前所选数据？').then(() => {
          this.deleteItems(this.selection.map(item => item.tnRcxjgjid.toString()))
        }).catch(err => {})
      } else {
    ...
    _DataGridDblClick(row) {
      ...
      this.$router.push({ name: 'DataFireLinkDetail', query: {btxxxlx : row.tcLbxxxlx, mxxxxlx: row.tcMxxxxlx}}) 
    },
    ...
    async saveListData () {
      ...
      if(this.formData.btlx===''){
        ...
        let obj1 = this.options['100201'].find(item=>{
          return item.tcSjubm === this.formData.tcMxxxxlx
        })
        //console.log(obj1, 'obj1')
        this.formData.tcMxxxxmc=obj1.tcSjnr

        let obj2 = this.options['100206'].find(item=>{
          return item.tcSjubm === this.formData.tcLbxxxlx
        })
        console.log(obj2, 'obj2')
        this.formData.tcLbxxxmc=obj2.tcSjnr

        
        //this.formData.tcLbxxxmc='消防安全设备运行及故障情况'
