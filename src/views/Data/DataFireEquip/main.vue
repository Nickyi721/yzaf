...
export default {
  ...
  methods: {
    ...
    $init (options) {
      if (options ) {
        this.options.type = options['100206'].concat(options['100201']).concat(options['100205'])
    ...
    // 保存信息项
    async saveListData () {
      const deleteIds = this.originList.filter(item => this.list.findIndex(row => row.tnRcxjjbxxszid === item.tnRcxjjbxxszid) < 0).map(item => item.tnRcxjjbxxszid)

      const updateList = this.list.map((row, index) => {
        const item = {
          tnXh: row.tnXh,                   // 序号
          tnRcxjjbxxszid: row.tnRcxjjbxxszid,     // 主键id
          tcXxxlx: row.tcXxxlx,             // 信息项类型
          tcXxxlxmc: row.tcXxxlxmc,         // 信息项类型名称
          tcXxxmc: row.tcXxxmc,             // 信息项名称
          tnXxxlrfs: row.tnXxxlrfs,         // 信息项录入方式
          tnSfxs: row.tnSfxs,         //信息项是否显示
