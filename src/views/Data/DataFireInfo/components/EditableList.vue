  ...
  <div class="common-editable">
    <el-table stripe border height="100%" :data="list" size="mini" class="grid-table" header-cell-class-name="grid-table-hd" @selection-change="_TableSelectionChange">
      ...
      <el-table-column prop="tcXxxmc" label="名称" min-width="200px">
        <input type="text" slot-scope="scope" maxlength="60" v-model="scope.row['tcXxxmc']" @change="e => valid(e, scope.row, 'tcXxxmc')" @blur="e => valid(e, scope.row, 'tcXxxmc')" :class="getAlertClass(scope.row, 'tcXxxmc')"></input>
      </el-table-column>
      ...
      <el-table-column prop="tcMxlx" label="明细类型值" width="150px">
        <input type="text" slot-scope="scope" maxlength="20" v-model="scope.row['tcMxlx']" :disabled="scope.row['tnXxxlrfs'] !== 7" @change="e => valid(e, scope.row, 'tcMxlx')" @blur="e => valid(e, scope.row, 'tcMxlx')" :class="getAlertClass(scope.row, 'tcMxlx')"></input>
      </el-table-column>
      ...
      <el-table-column prop="tcXxxxxxnr" label="选择项内容" width="120px">
        <input type="text" slot-scope="scope" maxlength="20" v-model="scope.row['tcXxxxxxnr']" :disabled="scope.row['tnXxxlrfs'] !== 2 && scope.row['tnXxxlrfs'] !== 3 && scope.row['tnXxxlrfs'] !== 4"></input>
      </el-table-column>
      <el-table-column prop="tnXxxldxid" label="联动项" width="150px">
        <select slot-scope="scope" v-model="scope.row['tnXxxldxid']" @change="_SelectLinkChange(scope.row)">
          <option v-for="option in notSelect.concat(list.filter(item => item.tnXh !== scope.row.tnXh).filter(item => item.tnXxxlrfs === 2 || item.tnXxxlrfs === 3 || item.tnXxxlrfs === 4))" :value="option.tnXh">{{option.tcXxxmc}}</option>
        </select>
      </el-table-column>
      ...
      <el-table-column prop="tcXxxldglz" label="联动项关联值" width="150px">
        <input type="text" slot-scope="scope" maxlength="20" v-model="scope.row['tcXxxldglz']" :disabled="scope.row['tnXxxldxid'] === -1 || scope.row['tnXxxldfs'] === 0" @change="e => valid(e, scope.row, 'tcXxxldglz')" @blur="e => valid(e, scope.row, 'tcXxxldglz')" :class="getAlertClass(scope.row, 'tcXxxldglz')"></input>
      </el-table-column>
      ...
      <el-table-column prop="tnSfxs" label="是否显示" align="center" width="75px">
        <input slot-scope="scope" type="checkbox" name="" v-model="scope.row['tnSfxs']" @change="_CheckChange(scope.row)">
      </el-table-column>
      <el-table-column prop="tnLk" label="列宽" min-width="150px">
        <input type="text" slot-scope="scope" maxlength="5" v-model="scope.row['tnLk']" @change="e => valid(e, scope.row, 'tnLk')" @blur="e => valid(e, scope.row, 'tnLk')" :class="getAlertClass(scope.row, 'tnLk')"></input>
      </el-table-column>
  ...
  data () {
    return {
      options: {
        ...
        tnXxxlrfs: [
          { tcSjubm: 0, tcSjnr: '上传图片' },
          { tcSjubm: 1, tcSjnr: '填写' },
          { tcSjubm: 2, tcSjnr: '单选' },
          { tcSjubm: 3, tcSjnr: '多选' },
          { tcSjubm: 4, tcSjnr: '下拉' },
          { tcSjubm: 5, tcSjnr: '日期' },
          { tcSjubm: 6, tcSjnr: '上传文件' },
          { tcSjubm: 7, tcSjnr: '明细' },
          { tcSjubm: 8, tcSjnr: '人员选择' },
          { tcSjubm: 9, tcSjnr: '机构选择' },
        ],
        tnXxxldfs: [
