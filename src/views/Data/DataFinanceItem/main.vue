  ...
  methods: {
    ...
    $init (options) {
      if (options && options['0301']) {
        this.options.type = options['0301']
    ...
    _ButtonSaveClick () {
      ...
      } else if (this.list.filter(item => (item.tnXxxlrfs !== 1 && item.tnXxxlrfs !== 5 && item.tnXxxlrfs !== 6)).filter(item => item.tnXxxldfs !== 0).filter(item => !item.tcXxxxxxnr ).length) {
        this.$msgbox({
    ...
    // 保存信息项
    async saveListData () {
      const deleteIds = this.originList.filter(item => this.list.findIndex(row => row.tnAfabszid === item.tnAfabszid) < 0).map(item => item.tnAfabszid)

      const updateList = this.list.map((row, index) => {
        const item = {
          tnXh: row.tnXh,                   // 序号
          tnAfabszid: row.tnAfabszid,     // 主键id
