  ...
  <div class="common-grid" ref="container" :style="{ 'paddingBottom': $slots.default ? '40px' : '0px'}">
    <el-table
      ...
      <el-table-column label="操作" width="150px">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="_downloadClick(scope.row.tcWjm, scope.row.tcYwjm)">下载</el-button>
          <el-button size="mini" type="primary" @click="_checkClick(scope.row.tnZlkid)">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods:{
    _downloadClick(id, tcYwjm){
      this.$emit("download",id, tcYwjm)
    },
    _checkClick(id){
      this.$emit("check",id,true)
    },
