    ...
    <el-table class="grid-table" header-cell-class-name="table-hd" stripe :border="border" highlight-current-row size="mini" height="100%" :data="list" :row-class-name="rowClickStyle ? 'click-row' : ''" @selection-change="_TableSelectionChange"  @row-dblclick="_TableDblClick">
      ...
      <el-table-column label="状态" prop="tnSfyx" width="200px">
        <template slot-scope="scope">
          <!-- 10030401：未生成 10030402：启用 10030403：停用 -->
          ...
          <el-switch 
            ...
            @change=" (val) => _SwitchChange(val, scope.row.tnEwmid, scope.row.tcEwmDlbm)"
          >
          </el-switch>
        </template>
      </el-table-column>
      <el-table-column label="操作">
         <template slot-scope="scope">
          <el-button v-if="scope.row.tnSfyx === 10030401" size="mini" type="primary" @click="_generateQrcode(scope.row)">生成</el-button>
          <el-button v-else-if="scope.row.tnSfyx === 10030402" size="mini" type="primary" @click="_generateQrcode(scope.row)">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
     _SwitchChange(val, id, dlbm) {
      this.$emit("handleQrcode", val, id, dlbm)
    },
    _generateQrcode(row) {
