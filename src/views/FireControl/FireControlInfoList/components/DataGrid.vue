<template >
  <div class="common-grid" ref="container" :style="{ 'paddingBottom': $slots.default ? '40px' : '0px'}">
    <el-table class="grid-table" header-cell-class-name="table-hd" stripe :border="border" highlight-current-row size="mini" height="100%" :data="list" :row-class-name="rowClickStyle ? 'click-row' : ''" @selection-change="_TableSelectionChange" @row-dblclick="_TableDblClick">
      <el-table-column v-show="disabled" type="index"></el-table-column>
      <el-table-column v-if="!disabled" type="selection"></el-table-column>
      
      <el-table-column v-for="(col, index) in config" v-if="col.tnXxxlrfs !== 0 && col.tnXxxlrfs !== 6" align="center" :key="index" :label="col.tcXxxmc" :prop="col.tnXfaqjbxxszid.toString()" :min-width="260 + 'px'">
        <template slot-scope="scope">
          <el-button v-if="col.tcMxlx" size="mini" type="primary" @click.stop="_ViewDetail(col, scope.row)">查看明细</el-button>
