  ...
  <div>
    <el-form ref="form" label-position="top" :model="bindData" :rules="rules" :label-width="labelWidth" class="common-form">
      <el-form-item v-for="(item, idx) in config" :key="idx" :label="item.tcXxxmc" :prop="item.tnRcxjjbxxszid.toString()" v-if="setLinkShow(item, idx)">
        <el-input :disabled="bool"  v-if="item.tnXxxlrfs === 1" v-model="bindData[item.tnRcxjjbxxszid]" size="mini" :type="item.tnXxxsjlx === 1 ? 'text' : 'number'" :placeholder="'请输入' + item.tcXxxmc" :maxlength="Math.floor(item.tnXxxsjcd/3)"></el-input>
        <el-radio-group :disabled="bool"  v-if="item.tnXxxlrfs === 2" v-model="bindData[item.tnRcxjjbxxszid]" @change="_OptionChange(item)">
          <el-radio v-for="option in options[item.tcXxxxxxnr]" :key="option.tcSjubm" :label="option.tcSjubm">{{option.tcSjnr}}</el-radio>
        </el-radio-group>

        <el-checkbox-group disabled  v-if="item.tnXxxlrfs === 3" v-model="bindData[item.tnRcxjjbxxszid]"  @change="_OptionChange(item)">
          <el-checkbox v-for="option in options[item.tcXxxxxxnr]" :key="option.tcSjubm" :label="option.tcSjubm">{{option.tcSjnr}}</el-checkbox>
        </el-checkbox-group>
        <el-select disabled  v-if="item.tnXxxlrfs === 4" v-model="bindData[item.tnRcxjjbxxszid]" :placeholder="'请选择' + item.tcXxxmc" size="mini" :disabled="item.disable" @change="_OptionChange(item)">
          <el-option v-for="option in options[item.tcXxxxxxnr]" :key="option.tcSjubm" :label="option.tcSjnr" :value="option.tcSjubm"></el-option>
        </el-select>
        <el-date-picker :disabled="bool"   v-if="item.tnXxxlrfs === 5" v-model="bindData[item.tnRcxjjbxxszid]" size="mini" :placeholder="'请选择' + item.tcXxxmc" value-format="yyyy-MM-dd"></el-date-picker>
        <el-switch  :disabled="bool"  v-if="item.tnXxxlrfs === 6"
          v-model="switchValue"
          active-text="启用"
          :active-value=0
          inactive-text="停用"
