  ...
  >
    ...
    >
      ...
      <el-table-column label="功能检查" width="100px">
        <el-button
          size="mini"
          v-if="scope.row.tcGnjcxxxlx"
          slot-scope="scope"
          @click.stop="_ViewDetail(scope.row)"
        >功能检查</el-button>
      </el-table-column>
      <el-table-column label prop="tnGnjcwxzt" width="100px">
        <template slot-scope="scope" v-if="scope.row.tcGnjcxxxlx">
          <p v-if="scope.row.tnGnjcwxzt===0">未提交</p>
          <p v-else-if="scope.row.tnGnjcwxzt===1">已提交</p>
          <p v-else-if="scope.row.tnGnjcwxzt===2">正常</p>
        </template>
      </el-table-column>
      ...
      <el-table-column label="维修状态" prop="tnWxzt" :min-width="300 + 'px'">
        <template slot-scope="scope">
          <p v-if="scope.row.tnWxzt===0">未提交</p>
          <p v-else-if="scope.row.tnWxzt===1">已提交</p>
          <p v-else-if="scope.row.tnWxzt===2">正常</p>
        </template>
      </el-table-column>
      <el-table-column  label="检查人" prop="tcCjrmc"  :min-width="150 + 'px'"></el-table-column>
      <el-table-column  label="创建时间" prop="tdCjsj"  :min-width="200 + 'px'"></el-table-column>
      <el-table-column
        v-for="(col, index) in config"
        :key="index"
        :label="col.tcXxxmc"
        :prop="col.tnRcxjjbxxszid.toString()"
        :width="col.tnLk+'px'"
        :min-width="300 + 'px'"
      ></el-table-column>
    ...
    _ViewDetail(row) {
      this.$router.push({
        ...
        query: {
          ...
          equipTypeText: this.currentTypeText || "消防安全设备运行及故障情况",
