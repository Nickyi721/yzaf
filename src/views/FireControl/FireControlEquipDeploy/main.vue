  ...
  methods: {
    $init(options) {
      if (options && options["100206"]) {
        ...
        this.equipType = this.equipTypeSet.filter(
          item => item.tcSjubm !== "030108" && item.tcSjubm !== "030109"
        );
      ...
      */
      ...
      if (this.type == 2) {
        ...
        if (this.currentType === "030106") {
          ...
          this.detailTypeText = this.equipTypeSet.filter(
            item => item.tcSjubm === this.detailType
          )[0].tcSjnr;
    ...
    // 事件 - 新增按钮点击
    // _ButtonAddClick () {
    //   console.log( this.currentType,'add')
    //   this.$router.push({
    //     path: '/firecontrol/equip/detail',
    //     query: {
    //       type: 'add',
