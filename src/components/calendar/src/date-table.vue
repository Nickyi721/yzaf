...
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, getI18nSettings, validateRangeInOneMonth } from '@commons/date-util';
export default {
  ...
  methods: {
    ...
    getCellClass({ text, type}) {
      ...
      if (type === 'current') {
        ...
        const curMonth = fecha.format(new Date(), 'yyyy-MM')
        ...
        if (curMonth === this.curMonthDatePrefix){
          classes.push('is-curMonth');
        }
      }
      return classes;
    },
